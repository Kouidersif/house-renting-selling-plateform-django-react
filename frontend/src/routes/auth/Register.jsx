import { Link } from "react-router-dom"
import { ButtonRadius } from "../../utils/exporter"
import InputField from "../../utils/inputs/InputField"
import propTypes from "prop-types";
import landScapeBuildings from "../../assets/images/auth-images/landscape-buildings.jpg";
import useAuthenticate from "../../hooks/auth/useAuthenticate";
import useGetAppContext from "../../../context/useGetAppContext";
import { useState } from "react";


const Register = ({ urls }) => {
    const { registrationFunc } = useAuthenticate()
    const [userFullName, setUserFullName] = useState("")
    const [userEmail, setUserEamil] = useState("")
    const [userPwd, setUserPwd] = useState("")
    const [userType, setUserType] = useState("")
    const { setErrorApi } = useGetAppContext()



    const handleSubmitButton = (e) => {
        e.preventDefault();
        if ( !userFullName & !userEmail & !userPwd & !userType ){
            return setErrorApi("All fields are required!")
        }
        const [firstName, lastName] = userFullName.split(" ");
        const data = {
            email : userEmail,
            password : userPwd,
            first_name: firstName, 
            last_name : lastName,
            is_tenant : userType === "tenant",
            is_landlord : userType === "landlord"
        }
        registrationFunc(data)
        
    }
    return (
        <div className="w-[95%] xl:w-[80%] mx-auto flex py-8 lg:py-16 px-4">
            <form className="w-full lg:w-1/2">
                <div className="w-[80%] mx-auto">
                    <Link to={urls.home} className="flex items-center gap-2 cursor-pointer">
                        <div className="bg-primary h-14 w-14 rounded-full flex items-center justify-center">
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                                width={24}
                                height={24}
                                viewBox="0 0 24 24"
                                fill="none"
                            >
                                <g clipPath="url(#clip0_2_3)">
                                    <path
                                        d="M23.715 9.16499L12.465 0.164989C12.332 0.0584866 12.1667 0.000457764 11.9963 0.000457764C11.8258 0.000457764 11.6605 0.0584866 11.5275 0.164989L0.2775 9.16499C0.156135 9.26236 0.0680565 9.39506 0.025468 9.54471C-0.0171205 9.69436 -0.0121105 9.85356 0.0398037 10.0002C0.091718 10.1469 0.187966 10.2738 0.315214 10.3634C0.442461 10.4529 0.594407 10.5006 0.75 10.5H2.25V23.25C2.25 23.4489 2.32902 23.6397 2.46967 23.7803C2.61032 23.921 2.80109 24 3 24H18V22.5H15V15.75C15 15.5511 14.921 15.3603 14.7803 15.2197C14.6397 15.079 14.4489 15 14.25 15H9.75C9.55109 15 9.36032 15.079 9.21967 15.2197C9.07902 15.3603 9 15.5511 9 15.75V22.5H3.75V10.5H16.5V8.99999H2.8875L12 1.70999L21.1125 8.99999H21C20.8011 8.99999 20.6103 9.07901 20.4697 9.21966C20.329 9.36031 20.25 9.55108 20.25 9.74999V14.25H21.75V10.5H23.25C23.405 10.4991 23.5558 10.4502 23.6819 10.3601C23.8079 10.27 23.903 10.143 23.9539 9.9967C24.0049 9.85037 24.0093 9.69184 23.9665 9.54291C23.9237 9.39398 23.8359 9.26195 23.715 9.16499ZM10.5 16.5H13.5V22.5H10.5V16.5Z"
                                        fill="white"
                                    />
                                    <path
                                        d="M18.75 19.9425L17.7825 18.975L16.725 20.0325L18.225 21.5325C18.3655 21.6722 18.5556 21.7506 18.7537 21.7506C18.9519 21.7506 19.142 21.6722 19.2825 21.5325L23.7825 17.0325L22.725 15.975L18.75 19.9425Z"
                                        fill="white"
                                    />
                                </g>
                                <defs>
                                    <clipPath id="clip0_2_3">
                                        <rect width={24} height={24} fill="white" />
                                    </clipPath>
                                </defs>
                            </svg>

                        </div>
                        <span className="text-xl font-semibold">RealEstate</span>
                    </Link>
                </div>
                <div className="w-[80%] 2xl:py-16 mx-auto flex flex-col gap-12">
                    <h2 className="text-center md:text-start">Get Started Now</h2>
                    <div className="flex flex-col gap-6">
                        <InputField value={userFullName} onChangeFunc={setUserFullName} placeHolder={"Enter your name"} labelName={"Name"} inputType={"text"} inputID={"userFullName"} />
                        <InputField value={userEmail} onChangeFunc={setUserEamil} placeHolder={"Enter your email"} labelName={"Email address"} inputType={"email"} inputID={"userEmail"} />
                        <InputField value={userPwd} onChangeFunc={setUserPwd} placeHolder={"Enter your password"} labelName={"Password"} inputType={"password"} inputID={"userPassword"} />

                        <ul className="grid w-full gap-6 grid-cols-1 xl:grid-cols-2">
                            <li>
                                <input
                                    type="radio"
                                    id="hosting-small"
                                    name="hosting"
                                    className="hidden peer"
                                    value="landlord"
                                    onChange={(e)=> setUserType(e.target.value)}
                                />
                                <label
                                    htmlFor="hosting-small"
                                    className="inline-flex items-center justify-between w-full p-5 text-gray-500 bg-white border border-gray-200 rounded-lg cursor-pointer peer-checked:border-blue-600 peer-checked:border-4 peer-checked:text-blue-600 hover:text-gray-600 hover:bg-gray-100"
                                >
                                    <div className="block text-purple-heart">
                                        <div className="w-full text-lg font-semibold">LandLord</div>
                                        <div className="w-full">I have properties to rent/sell</div>
                                    </div>
                                    <svg
                                        className="w-5 h-5 ms-3 rtl:rotate-180"
                                        aria-hidden="true"
                                        xmlns="http://www.w3.org/2000/svg"
                                        fill="none"
                                        viewBox="0 0 14 10"
                                    >
                                        <path
                                            stroke="currentColor"
                                            strokeLinecap="round"
                                            strokeLinejoin="round"
                                            strokeWidth={2}
                                            d="M1 5h12m0 0L9 1m4 4L9 9"
                                        />
                                    </svg>
                                </label>
                            </li>
                            <li>
                                <input
                                    type="radio"
                                    id="hosting-big"
                                    name="hosting"
                                    value="tenant"
                                    onChange={(e)=> setUserType(e.target.value)}
                                    className="hidden peer"
                                />
                                <label
                                    htmlFor="hosting-big"
                                    className="inline-flex items-center justify-between w-full p-5 text-gray-500 bg-white border border-gray-200 rounded-lg cursor-pointer peer-checked:border-razzmatazz peer-checked:border-4 peer-checked:text-blue-600 hover:text-gray-600 hover:bg-gray-100"
                                >
                                    <div className="block text-razzmatazz">
                                        <div className="w-full text-lg font-semibold">Tenant/Buyer</div>
                                        <div className="w-full">I’m looking for a place to live</div>
                                    </div>
                                    <svg
                                        className="w-5 h-5 ms-3 rtl:rotate-180"
                                        aria-hidden="true"
                                        xmlns="http://www.w3.org/2000/svg"
                                        fill="none"
                                        viewBox="0 0 14 10"
                                    >
                                        <path
                                            stroke="currentColor"
                                            strokeLinecap="round"
                                            strokeLinejoin="round"
                                            strokeWidth={2}
                                            d="M1 5h12m0 0L9 1m4 4L9 9"
                                        />
                                    </svg>
                                </label>
                            </li>
                        </ul>

                        <div className="flex gap-2 items-center">
                            <input type="checkbox" className="w-4 h-4" name="" id="" />
                            <label htmlFor="inputCheckBox">I agree to the terms & policy</label>
                        </div>
                        <ButtonRadius handleButtonClick={handleSubmitButton} text="Register" styleClass="bg-primary text-white py-2 mt-7 hover:bg-primary/90" />
                        <h4 className="text-center mt-2">Have an account ? <Link to={urls.login} className="text-blue-600">Login</Link></h4>
                    </div>
                </div>
            </form>
            <div className="hidden h-full border-2 md:flex-1 lg:block md:max-w-[80%] lg:max-w-[100%]">
                <img src={landScapeBuildings}
                    alt="" className="w-full h-full object-cover " />
            </div>

        </div>
    )
}

Register.propTypes = {
    urls: propTypes.object.isRequired,
}

export default Register